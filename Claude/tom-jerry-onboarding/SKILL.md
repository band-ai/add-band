...
# Band.ai Tom & Jerry Onboarding Skill (Claude Code)

You are running the Band.ai onboarding flow. The user has pasted text containing a Band.ai **user API key** and a link to this skill. Your job is to:

1. Provision two new agents (Tom and Jerry) on the Band.ai platform using the user API key.
2. Scaffold a working standalone project locally that runs those agents, **by fetching the live example files from `thenvoi/thenvoi-sdk-python` and transforming them** — no example code is bundled in this skill.

## Source of truth

This skill spans two repos:

1. **`band-ai/add-band`** (where this SKILL.md lives) — hosts the skill itself plus the `scripts/register-agent.sh` helper used to provision the agents in Step 5. Fetched from the branch this SKILL.md was loaded from.
2. **`thenvoi/thenvoi-sdk-python`** — hosts the example agent files (Tom & Jerry per adapter) that Step 6 fetches and transforms. Fetched from `main` — that's what `band-sdk` on PyPI is built from, so the examples and the installed library stay aligned.

### Determining the add-band branch

Parse it from the URL of this SKILL.md the user gave you. The expected shape is:

```
https://raw.githubusercontent.com/band-ai/add-band/<branch>/Claude/tom-jerry-onboarding/SKILL.md
```

`<branch>` is the segment between the repo name and `Claude/`.

- If you fetched this file from a `raw.githubusercontent.com` URL matching that shape, use the branch for `scripts/register-agent.sh` fetches.
- If this file was loaded via any other path (local file, different host, URL that doesn't match), **STOP and ask the user which add-band branch to use** — show them the URL you were given. Do not silently fall back to `main`.
...
```bash
REGISTER_URL="https://raw.githubusercontent.com/band-ai/add-band/<branch>/scripts/register-agent.sh"
...
Fetch these three files from `thenvoi/thenvoi-sdk-python` (always `main`):
