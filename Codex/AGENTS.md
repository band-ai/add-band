...
1. **`band-ai/add-band`** (where this AGENTS.md lives) — hosts the skill itself plus the `scripts/register-agent.sh` helper used to provision the agents in Step 5. Fetched from the branch this AGENTS.md was loaded from.
...
Parse it from the URL of this file the user gave you. The expected shape is:

```
https://raw.githubusercontent.com/band-ai/add-band/<branch>/Codex/AGENTS.md
```

`<branch>` is the segment between the repo name and `Codex/`.
