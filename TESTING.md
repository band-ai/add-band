...
The Band web app hands the user a `curl -fsSL <url> | bash` one-liner plus a Band **API
key** to copy. The web app does **not** edit the script — the bootstrap **prompts**
for the key (reading from `/dev/tty`) and the user pastes it. Pre-set `BAND_API_KEY`
to skip the prompt.
...
- **stdin is the pipe, not your terminal.** Under `curl … | bash`, stdin is the script
  ...
  instead of the user.
- **no placeholder substitution.** A bootstrap that doesn't acquire `BAND_API_KEY`
  itself (prompt, or accept a pre-set env var) never gets a key. `scripts/check.py`
  enforces that the snippet references `BAND_API_KEY`.
...
would — no key substitution; you'll be prompted (or pre-set `BAND_API_KEY`):
...
BAND_API_KEY="<your-band-api-key>" bash <(curl -fsSL "file://$PWD/<harness>/bootstrap.sh")
```
