...
hermes_project="$(hermes --version 2>&1 | sed -n 's/^Project: //p')"
hermes_python="$hermes_project/venv/bin/python"
...
bash "$("$hermes_python" -c 'import hermes_band_platform; print(hermes_band_platform.__path__[0])')/scripts/register-agent.sh" | sed 's/^export //' >> "$hermes_env"
