...
from driver.exchange import liveness_name, liveness_prompt, marker
...
async def test_unaddressed_turn_does_not_prompt_reply(
    ...
    # A natural name, not a control-code token: Hermes's prompt-injection guard
    # refuses to echo the latter, failing the liveness probe on a live model.
    control = liveness_name(run_id)
    ...
    async with owner_chat(pa) as chat:
        ...
        reply = await chat.ask(token=control, prompt=liveness_prompt(control))
        reply.assert_contains_any([control])
        unsolicited = said_by(
